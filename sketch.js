...
function preload(){
  ...
  benBoyImg = loadImage("Images/BenTennyson.png");
...
function draw(){
 ...
 if(fourArms.isTouching(drAnimoGroup)){
  ...
  if(lives>0 && lives === 20){
    fourArms.addImage("fourArms",benBoyImg);
    fourArms.velocityX = 0;
    fourArms.velocityY = 0;
    fourArms.x=300;
    fourArms.y=800;

    drAnimoGroup.destroyEach();
    drAnimoGroup.setVelocityXEach(0);

    omnimatrixGroup.destroyEach();
    omnimatrixGroup.setVelocityXEach(0);

    stroke("Black");
    strokeWeight(5);
    fill("Purple")
    textSize(70);
    text("OMNIMATRIX PROTECTED ",600,450);
  }
